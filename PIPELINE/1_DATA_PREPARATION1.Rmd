...
d0 <- read.delim("1_DATA/1_data_manual_selection.txt")
...
# 1.5 Change Y in integer

```{r}
n_col = dim(d3)[2]
d3[,n_col+1] = 1
names(d3)[n_col+1]='ISEDGE'

d3$ISEDGE[d3$CLASS=='INTERIOR'] = 1
d3$ISEDGE[d3$CLASS=='EDGE'] = 0

d4 = d3[,2:dim(d3)[2]]
```


# 1.6 Shuffle and split the data-set in Training and Test sets
...
write.table(train, file = "2_DATA/2_data_training.txt", sep = "\t", quote=FALSE, row.names=FALSE, col.names = colnames(train))
write.table(test, file = "2_DATA/2_data_test.txt", sep = "\t", quote=FALSE, row.names=FALSE, col.names = colnames(test))
