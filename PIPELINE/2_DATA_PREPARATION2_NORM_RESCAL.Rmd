...
iii) Save: transformed data set, mean and stdev of features to be used for testing and cross validation 
...
d0 <- read.delim("2_DATA/2_data_training.txt")
...
write.table(d3, file = "3_DATA/3_data_training_transf.txt", sep = "\t", quote=FALSE, row.names=FALSE, col.names = colnames(d3))
write.table(as.data.frame(FEATURES_MEAN) , file = "3_DATA/3_data_training_param_MEAN.txt", sep = "\t", quote=FALSE, row.names=F)
write.table(as.data.frame(FEATURES_STDEV), file = "3_DATA/3_data_training_param_STDEV.txt", sep = "\t", quote=FALSE, row.names=F)
